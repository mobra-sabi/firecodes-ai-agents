...
Răspunde DOAR cu JSON valid, fără text suplimentar."""
    ...
    async def _qwen_execute(self, question: str, context: List[Dict], web_context: List[Dict], site_url: str, mode: str) -> QwenExecution:
        ...
        learning_context = await self.qwen_memory.get_learning_context(domain, limit=3)
        
        # Construiește prompt-ul de bază
        base_prompt = f"""Ești Qwen 2.5, learning engine-ul și vocea site-ului {domain}.
...
Răspunde în română, ca reprezentant oficial al {domain}."""
        ...
        system_prompt = await self.qwen_memory.get_enhanced_prompt(domain, base_prompt, question)
        ...
        try:
            ...
            # Salvează conversația în memoria Qwen
