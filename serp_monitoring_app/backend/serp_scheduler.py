...
class SERPMonitor:
    ...
    async def _fetch_serp_results(
        ...
        for keyword in keywords:
            # Simulare API call (1 secundă delay)
            ...
            master_result = next((r for r in mock_results if master_domain in r["url"]), None)
            if master_result:
    ...
    def _generate_mock_serp(self, keyword: str, master_domain: str) -> List[Dict]:
        """
        Generate mock SERP results
        TODO: Replace cu Brave API
        """
        import random
        ...
        # Master domain apare la o poziție random (1-10)
        master_rank = random.randint(1, 10)
        ...
        for i in range(10):
            if rank == master_rank:
                # Insert master
                results.append({
                    "url": f"https://{master_domain}/page-{keyword.replace(' ', '-')}",
                    "title": f"{keyword.title()} - {master_domain}",
                    "snippet": f"Servicii de {keyword}...",
                    "type": "organic"
                })
            else:
                # Insert competitor
                ...
                results.append({
                    "url": f"https://{comp}/page-{keyword.replace(' ', '-')}",
                    "title": f"{keyword.title()} - {comp}",
                    "snippet": f"Oferim {keyword}...",
                    "type": "organic"
                })
            rank += 1
